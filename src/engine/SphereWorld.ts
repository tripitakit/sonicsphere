...
// Engine-balance weights: keep classic subtractive dominant, but ensure
// noise/FM/resonator are always present with comparable share.
...
export class SphereWorld {
  ...
  private balancedPoolByEngine(
    ...
    const cumulative: Array<{ engine: SoundEngineType; edge: number }> = [];
    let acc = 0;
    for (const engine of activeEngines) {
      acc += weights[engine] / totalWeight;
      cumulative.push({ engine, edge: acc });
    }

    const pool: number[] = [];
    for (let i = 0; i < count; i++) {
      const roll = rng();
      const selected = cumulative.find((entry) => roll <= entry.edge)
        ?? cumulative[cumulative.length - 1]!;
      const bucket = buckets[selected.engine];
      const archetypeIdx = bucket[Math.floor(rng() * bucket.length)];
      if (archetypeIdx !== undefined) {
        pool.push(archetypeIdx);
      }
    }

    // Spread clusters generated by weighted picks.
    for (let i = pool.length - 1; i > 0; i--) {
      const j = Math.floor(rng() * (i + 1));
      [pool[i], pool[j]] = [pool[j]!, pool[i]!];
    }
